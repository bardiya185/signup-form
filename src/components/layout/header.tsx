      ...
      <div className="flex h-[72px] items-center gap-3">
        ...
        <div className="mr-auto flex items-center gap-1">
          ...
          <div className="relative">
            <button onClick={()=>setCartOpen(!cartOpen)} className={`relative grid h-10 w-10 place-items-center rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 ${items.length?'animate-soft-pulse':''}`} aria-label="مشاهده سبد خرید"><ShoppingCart size={20}/>{items.length>0&&<b className="absolute -left-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-brand px-1 text-[10px] text-white">{items.reduce((sum,item)=>sum+item.quantity,0)}</b>}</button>
            {cartOpen && <div className="animate-rise-in absolute left-0 top-12 w-[335px] overflow-hidden rounded-2xl border bg-white text-right shadow-2xl dark:border-zinc-700 dark:bg-zinc-900">
