...
import { returnOrder } from '@/server/services/order.service';
...
export const POST = apiHandler(async (req: NextRequest, ctx: { params: Promise<{ orderNumber: string }> }) => {
  ...
  const data = validate(await req.json(), {
    reason: { required: true, min: 3, max: 300, label: 'دلیل مرجوعی' },
  });
  return ok(returnOrder(user, orderNumber, data.reason));
});
