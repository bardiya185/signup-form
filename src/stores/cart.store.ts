...
export const useCartStore = create<CartStore>()(persist((set) => ({
  items: [], wishlist: [], isCartOpen: false, hasHydrated: false,
  ...
  add: (product, opts) => set(state => {
    ...
    return { isCartOpen: true, items: found ? state.items.map(item => item===found ? {...item,quantity:item.quantity+1}:item) : [...state.items,{product,quantity:1,...opts}] };
  }),
